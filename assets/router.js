...
//
//         const loginItem=document.getElementById('login-btn');
//         const authElements=document.querySelectorAll('.auth-only');
//         const addBtn = document.querySelector('.add-btn');
//         if(auth.isLoggedIn){
//             if(loginItem) loginBtn.classList.add('hidden');
...
//
//     if (loginForm) {
//         loginForm.addEventListener('submit', async (e) => { // Використовуємо 'submit' на формі
//             e.preventDefault();
//
...
//                     await checkAuthStatus(); // Чекаємо оновлення інтерфейсу
//                 } else {
//                     alert(result.message || "Неправильний логін або пароль");
//                 }
//             } catch (error) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const save_student_btn = document.getElementById("save-student-btn");
    const add_trigger_btn = document.querySelector('.add-btn');
    const student_id_input = document.getElementById("student-id");
    const delete_ok_btn = document.querySelector("#delete-warn-btn .ok-btn");
    ...
    async function checkAuthStatus() {
        try {
            ...
            if (auth.isLoggedIn) {
                ...
                authElements.forEach(el => el.classList.remove('hidden'));
                if (window.location.hash === "#students") loadStudentsFromServer();
            } else {
                if (loginItem) loginItem.classList.remove('hidden');
                authElements.forEach(el => el.classList.add('hidden'));
                if (["#students", "#tasks", "#messages"].includes(window.location.hash)) {
    ...
    if (table_body) {
        table_body.addEventListener("click", (event) => {
            ...
            if (edit_btn) {
                row_edit = edit_btn.closest("tr");
                const cells = row_edit.getElementsByTagName("td");

                document.getElementById("modal-title").textContent = "Edit Student";
                if (save_student_btn) save_student_btn.textContent = "Save";
                student_id_input.value = "editing";

                const fullName = cells[2].textContent.trim().split(" ");
                document.getElementById("student-group").value = cells[1].textContent.trim();
                document.getElementById("student-first-name").value = fullName[0] || "";
                document.getElementById("student-last-name").value = fullName[1] || "";

                const gender = cells[3].textContent.trim();
                document.getElementById("student-gender").value = (gender === "F" || gender === "Female") ? "Female" : "Male";
                document.getElementById("student-birthday").value = cells[4].textContent.trim();
            }

            if (delete_btn) {
                row_delete = delete_btn.closest("tr");
                const studentName = row_delete.getElementsByTagName("td")[2].textContent;
                const warningText = document.querySelector("#delete-warn-btn .warning-text");
                if (warningText) warningText.textContent = `Are you sure you want to delete ${studentName}?`;
            }
        });
    }

    if (save_student_btn) {
        save_student_btn.addEventListener("click", async (e) => {
            e.preventDefault();
            console.log("Кнопка Save натиснута");

            const isNotLoggedIn = !loginItem.classList.contains('hidden');
            if (isNotLoggedIn) {
            ...
            const isEditing = student_id_input.value === "editing";
            let url = `api/index.php?action=${isEditing ? 'editStudent' : 'addStudent'}`;

            if (isEditing && row_edit) {
                url += `&id=${row_edit.getAttribute('data-id')}`;
            }

            console.log("Відправка запиту на:", url, studentData);
            ...
            try {
                ...
                if (result.success) {
                    ...
                    alert("Помилка: " + (result.errors ? result.errors.join(", ") : result.message));
                }
            } catch (error) {
                console.error("Fetch error:", error);
                alert("Критична помилка з'єднання з сервером");
            }
        });
    }

    if (delete_ok_btn) {
        delete_ok_btn.addEventListener("click", async (e) => {
            e.preventDefault();
            if (!row_delete) return;

            const id = row_delete.getAttribute('data-id');
            try {
                const response = await fetch(`api/index.php?action=deleteStudent&id=${id}`, { method: 'DELETE' });
                const result = await response.json();
    ...
    async function loadStudentsFromServer() {
        ...
        students.forEach(s => {
            ...
            <tr data-id="${s.id}">
                ...
                <td>${s.birthday}</td>
                <td><span class="student-status online"></span></td>
                <td>
                    <a href="#student-modal" class="btn edit-btn"><i class="fa-regular fa-pen-to-square"></i></a>
                    <a href="#delete-warn-btn" class="delete-btn btn"><i class="fa-solid fa-xmark"></i></a>
                </td>
            </tr>`;
        });
    }
    ...
    function clearForm() {
        ...
        document.getElementById("modal-title").textContent = "Add student";
        if (save_student_btn) save_student_btn.textContent = "Create";
        student_id_input.value = "";
        row_edit = null;
        ["student-first-name", "student-last-name", "student-birthday"].forEach(id => {
            const input = document.getElementById(id);
            if (input) input.value = "";
        });
    }

    add_trigger_btn?.addEventListener("click", clearForm);
    document.querySelectorAll('.close-btn, .cancel-btn, a[href="#students"]').forEach(b => b.addEventListener('click', clearForm));
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', async (e) => {
            ...
            if (res.success) { window.location.hash = "#students"; checkAuthStatus(); }
